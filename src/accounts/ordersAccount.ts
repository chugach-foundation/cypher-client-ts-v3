...
import { getQuoteFromBase, splToUiPrice, splToUiAmount } from '../utils';
...
export class DerivativesOrdersAccount {
  ...
  getPriceQuantity(
    ...
    const quoteM = viewer.market.state.inner.quoteMultiplier;
    const baseQty = new BN(leafNode.baseQuantity.toNumber()).mul(baseM);
    const quoteQty = getQuoteFromBase(
      baseQty,
      leafNode.getPrice(),
      baseM,
      quoteM
    );
    const price = splToUiPrice(
      quoteQty.div(baseQty),
